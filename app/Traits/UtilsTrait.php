...
trait UtilsTrait
{
     ...
     * @param $args
     */
    public function exchangeArray($args)
    {
        foreach ($args as $argKey => $val) {
            $action = "set" . ucfirst($argKey);
            $this->$action($val);
        }
    }

    /**
     * Get currency by domain name
    ...
    public function registerNewPermissions()
    {
        foreach ($this->getControllersList() as $controller => $permissions) {
            foreach ($permissions as $permission) {
                Permission::findOrCreate($permission['permissionName']);
            }
        }
    }
