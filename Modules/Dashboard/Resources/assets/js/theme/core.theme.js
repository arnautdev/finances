...
var handleSortable = function () {
    ...
    if ($('table.sortable').length !== 0) {
        var $saveOrdUrl = $('table.sortable').data('save-order');
        $('table.sortable tbody').sortable({
            stop: function (event, ui) {
                var $order = [];
                $('table.sortable tbody tr').each(function (index, el) {
                    $order[index] = $(el).data('id');
                });

                console.log($saveOrdUrl);
                $.post($saveOrdUrl, {'order': $order}, function ($resp) {
                    console.log($resp);
                });
            }
        });
    ...
    if ($('.gallery-sortable').length !== 0) {
        var $saveOrdUrl = $('.gallery-sortable').data('save-order');
        $('.gallery-sortable').sortable({
            stop: function (event, ui) {
                var $order = [];
                $('.gallery-sortable div').each(function (index, el) {
                    $order[index] = $(el).data('id');
                });
...
// stay active tabs
var stayActiveTabs = function () {
    'use strict';
    $('a[data-toggle="tab"]').on('click', function (e) {
        var $activeTab = $(e.target).attr('href');
        if ($activeTab === undefined) {
            $activeTab = $(e.target).parent('a').attr('href');
        }
...
var checkAllPermissions = function () {
    'use strict';
    if ($('#permissions-table').length !== 0) {
        $('input.main-permission-row').on('change', function () {
            var $class = $(this).attr('id');
            if ($(this).is(':checked')) {
                $('.' + $class).removeAttr('checked');
                $('.' + $class).attr('checked', 'checked');
            } else {
                $('.' + $class).removeAttr('checked');
            }
        });
...
var dataTables = function () {
    ...
    if ($('table.data-table').length !== 0) {
        ...
        // $('table.data-table thead tr').clone(true).appendTo('table.data-table thead');
        // $('table.data-table thead tr:eq(1) th').each(function (i) {
        //     var title = $(this).text();
        //     $(this).html('<input type="text" placeholder="Search ' + title + '" class="form-control no-radius" />');
        //
...
var tagItInitFun = function () {
    'use strict';
    if ($('.jquery-tagIt').length !== 0) {
        $('.jquery-tagIt').tagit({
            fieldName: $('.jquery-tagIt').data('name') + '[]'
        });
    }
...
var handleSelect2 = function () {
    ...
    if ($('.multiple-select2').length !== 0) {
        var $placeholder = $(".multiple-select2").data('placeholder')
        $(".multiple-select2").select2({placeholder: $placeholder});
    }
};
